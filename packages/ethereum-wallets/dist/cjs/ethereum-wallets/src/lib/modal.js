...
exports.createTxModal = createTxModal;
exports.createChainSwitchModal = createChainSwitchModal;
const viem_1 = require("viem");
const utils_1 = require("./utils");
const styles_1 = require("./styles");
function createTxModal({ onCancel, txs, relayerPublicKey, explorerUrl, }) {
    // Create a style element and append the CSS styles
    const styleElement = window.document.createElement("style");
    styleElement.textContent = styles_1.modalStyles;
    ...
    const isOnboard = txs.find((tx) => tx.actions[0].type === "AddKey" &&
    ...
    ${txs.length === 1 && isLogIn
        ...
        : txs.length === 1 &&
            ...
            ? "<h2>Log out</h2>"
            : isLogIn
                ? `<h2>Log in: send ${txs.length} transaction${txs.length > 1 ? "s" : ""}</h2>`
  ...
  `;
    ...
    const renderTxs = ({ selectedIndex, ethTxHashes, error, onConfirm, }) => {
        const container = document.querySelector(".ethereum-wallet-txs");
        container.innerHTML = "";
        txs.forEach((tx, i) => {
            const txNumber = i + 1;
            ...
            const explorerLink = i < ethTxHashes.length
              ...
              <div class="ethereum-wallet-tx-params">
                ${tx.actions[0].type === "AddKey"
                          ...
                          <p>WARNING: The application is requesting a FullAccess key. You can lose your account and all your assets. Only approve this transaction if you know what you are doing!<p>
                            ...
                            : tx.actions[0].params.accessKey.permission.allowance ===
                                ...
                                tx.actions[0].params.accessKey.permission
                                    .methodNames[0] === utils_1.RLP_EXECUTE
                                ? `
                        <div class="ethereum-wallet-tx-info-text">
                          <p>This one-time transaction will onboard your account and enable you to transact on NEAR Protocol.</p>
                                ...
                                : `
                      <div class="ethereum-wallet-tx-info-text">
                        <p>Connect to ${tx.actions[0].params.accessKey.permission.receiverId}</p>
                        <p>
                          Network Fee Allowance: ${tx.actions[0].params.accessKey.permission
                                    .allowance === "0"
                                    ? "unlimited"
                                    : (0, viem_1.formatUnits)(BigInt(tx.actions[0].params.accessKey.permission
                                        .allowance ?? utils_1.DEFAULT_ACCESS_KEY_ALLOWANCE), 24)} NEAR
                        </p>
                        ${tx.actions[0].params.accessKey.permission
                                    .allowance === "0"
                    ...
                    <dl>
                      ...
                      <div class="ethereum-wallet-tx-info-row">
                        <dt>Type</dt>
                        <dd class="ethereum-wallet-tx-highlight">${tx.actions[0].params.methodName}</dd>
                      </div>
                      ${tx.actions[0].params.deposit === "1" ||
                          ...
                          <div class="ethereum-wallet-tx-info-row">
                            <dt>Deposit</dt>
                            <dd class="ethereum-wallet-tx-highlight">${(0, viem_1.formatUnits)(BigInt(tx.actions[0].params.deposit), 24)} NEAR</dd>
                          </div>
                        `}
                    </dl>
                    `
                      ...
                      <dl>
                        ...
                        <div class="ethereum-wallet-tx-info-row">
                          <dt>Type</dt>
                          <dd class="ethereum-wallet-tx-highlight">${tx.actions[0].type}</dd>
                        </div>
                        <div class="ethereum-wallet-tx-info-row">
                          <dt>Amount</dt>
                          <dd class="ethereum-wallet-tx-highlight">${(0, viem_1.formatUnits)(BigInt(tx.actions[0].params.deposit), 24)} NEAR</dd>
      ...
      `;
        ...
        const toggleButton = window.document.querySelector(".ethereum-wallet-btn-details");
        const detailsContainer = window.document.querySelector(".ethereum-wallet-txs-details");
...
function createChainSwitchModal({ chain }) {
    // Create a style element and append the CSS styles
    const styleElement = window.document.createElement("style");
    styleElement.textContent = styles_1.modalStyles;
    window.document.head.appendChild(styleElement);
