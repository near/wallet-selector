...
import { ETHEREUM_ACCOUNT_ABI, DEFAULT_ACCESS_KEY_ALLOWANCE, RLP_EXECUTE, MAX_TGAS, EthTxError, } from "./utils";
...
const EthereumWallets = async ({ id, options, store, provider, emitter, logger, params: { wagmiConfig, web3Modal, chainId, alwaysOnboardDuringSignIn = false, devMode, devModeAccount = "eth-wallet.testnet", nearNodeUrl, }, }) => {
    ...
    const nearProvider = new JsonRpcProvider(nearNodeUrl ??
        // @ts-expect-error
    ...
    const executeTransaction = async ({ tx, relayerPublicKey, }) => {
        ...
        switch (tx.actions[0].type) {
            ...
            case "DeleteAccount":
                break;
            case "AddKey": {
                const publicKey = bytesToHex(bs58.decode(tx.actions[0].params.publicKey.split(":")[1]));
    ...
    const signAndSendTransactions = async (transactions) => {
        ...
        if (accountLogIn.publicKey && nearTxs.length) {
            ...
            if (accessKeyUsable) {
                ...
                const signedTransactions = await signTransactions(nearTxs, signer, options.network);
                const results = [];
                    ...
                    try {
                        ...
                        for (const [index, tx] of txs.entries()) {
                                            ...
                                            renderTxs({
                                                ...
                                                onConfirm: async () => {
                                                    try {
                                                        ...
                                                        rejectTx(new EthTxError("Transaction request error."));
                                                    }
                                                },
                                            });
                                        });
                            ...
                            if (receipt.status !== "success") {
                                ...
                                if (failedOutcome) {
                                    reject(parseRpcError(failedOutcome.outcome.status.Failure));
                                }
                                else {
...
export function setupEthereumWallets(params) {
    return async () => {
        if (!wagmiCore) {
            if (params.wagmiCore) {
