...
export interface DelegateActionModalOptions extends Omit<ModalOptions, 'title'> {
    delegateAction: any; // DelegateAction type from @near-js/transactions
...
export class DelegateActionModal extends Modal {
    ...
    private render() {
        ...
        description.textContent = 'Review the delegate action details below, then sign with your biometric authentication.';
        ...
        if (delegateAction.actions && Array.isArray(delegateAction.actions)) {
            ...
            delegateAction.actions.forEach((action: any, index: number) => {
                ...
                if (action.functionCall) actionType = 'FunctionCall';
                else if (action.transfer) actionType = 'Transfer';
                else if (action.addKey) actionType = 'AddKey';
                else if (action.deleteKey) actionType = 'DeleteKey';
                else if (action.createAccount) actionType = 'CreateAccount';
                else if (action.deleteAccount) actionType = 'DeleteAccount';
                else if (action.deployContract) actionType = 'DeployContract';
                else if (action.stake) actionType = 'Stake';
                ...
                if (action.functionCall) {
                    ...
                    if (fc.gas !== undefined) {
                        this.addSmallDetailRow(actionDetails, 'Gas', this.formatGas(fc.gas));
                    }
                    if (fc.deposit !== undefined) {
                        this.addSmallDetailRow(actionDetails, 'Deposit', this.formatDeposit(fc.deposit));
                    }
                } else if (action.transfer) {
                    this.addSmallDetailRow(actionDetails, 'Amount', this.formatDeposit(action.transfer.deposit));
                }
        ...
        infoBox.textContent = 'ℹ️ Delegate actions allow another party to execute transactions on your behalf within specified limits.';
    ...
    private addSmallDetailRow(container: HTMLElement, label: string, value: string) {
        const row = document.createElement('div');
        row.style.display = 'flex';
        row.style.justifyContent = 'space-between';
    ...
    private formatGas(gas: string | bigint | number): string {
        const gasNum = typeof gas === 'string' ? BigInt(gas) : typeof gas === 'number' ? BigInt(gas) : gas;
    ...
    private formatDeposit(deposit: string | bigint | number): string {
        ...
        const depositStr = typeof deposit === 'string' ? deposit : deposit.toString();
    ...
    private async handleApprove() {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Delegate action signing failed';
