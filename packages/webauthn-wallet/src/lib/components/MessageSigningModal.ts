...
export interface MessageSigningModalOptions extends Omit<ModalOptions, 'title'> {
    message: string;
    recipient: string;
    nonce: Buffer;
    callbackUrl?: string;
...
export class MessageSigningModal extends Modal {
    ...
    private render() {
        ...
        description.textContent = 'Review the message details below, then sign with your biometric authentication.';
        ...
        infoBox.textContent = 'ℹ️ This is a NEP-413 message signature request. Your signature proves ownership without sending a transaction.';
