...
```bash
# Using Yarn
yarn add @near-wallet-selector/web3auth

# Using NPM.
npm install @near-wallet-selector/web3auth
```
...
import { setupWalletConnect } from "@near-wallet-selector/web3auth";
...
- `iconUrl` (`string?`): Image URL for the icon shown in the modal. This can also be a relative path or base64 encoded image. Defaults to `./assets/web3auth-icon.png`.
...
```ts
import { setupWalletConnect } from "@near-wallet-selector/web3auth";
import walletConnectIconUrl from "@near-wallet-selector/web3auth/assets/web3auth-icon.png";
