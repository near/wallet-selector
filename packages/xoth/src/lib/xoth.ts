...
interface XothWalletState {
  wallet: WalletConnection;
  keyStore: keyStores.BrowserLocalStorageKeyStore;
}

interface XothWalletExtraOptions {
  walletUrl: string;
}
...
const resolveWalletUrl = (network: Network, walletUrl?: string) => {
  ...
  switch (network.networkId) {
    case "mainnet":
      return "https://wallet.xoth.app";
    case "testnet":
      return "https://wallet.testnet.xoth.app";
    default:
...
const setupWalletState = async (
  params: XothWalletExtraOptions,
  network: Network
): Promise<XothWalletState> => {
...
const XothWallet: WalletBehaviourFactory<
  BrowserWallet,
  { params: XothWalletExtraOptions }
...
export function setupXothWallet({
  walletUrl,
  iconUrl = icon,
  deprecated = false,
  ...
  return async () => {
    ...
    return {
      ...
      metadata: {
        name: "XothWallet",
        description: "Mobile wallet for NEAR Protocol & Octopus Network.",
        iconUrl,
        deprecated,
        available: true,
      },
