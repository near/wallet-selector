...
import type { InjectedTokenary } from "./injected-tokenary";

declare global {
  interface Window {
    near: InjectedTokenary | undefined;
  }
}

export interface TokenaryParams {
  iconUrl?: string;
}

interface TokenaryState {
  wallet: InjectedTokenary;
}

const isInstalled = () => {
  return waitFor(() => !!window.near?.isTokenary).catch(() => false);
};

const setupTokenaryState = (): TokenaryState => {
...
const Tokenary: WalletBehaviourFactory<InjectedWallet> = async ({
  options,
  metadata,
  store,
  emitter,
  ...
  const _state = setupTokenaryState();
...
export function setupTokenary({
  iconUrl = "./assets/tokenary-icon.png",
}: TokenaryParams = {}): WalletModuleFactory<InjectedWallet> {
  return async () => {
    const installed = await isInstalled();

    // Add extra wait to ensure Tokenary's sign in status is read from the
