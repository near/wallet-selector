...
import { Action, TxAction, FunctionCallTxAction } from "../actions";
...
function setupSenderWallet(): WalletModule<InjectedWallet> {
	return function SenderWallet({
		...
		const transformActions = (actions: Array<Action | TxAction>) => {
			...
			return actions.map((action) => {
				if (action instanceof transactions.Action) {
					...
					/// coerce naj functionCall to FunctionCallAction.params
					const txAction = action as unknown as FunctionCallTxAction;
					return {
						methodName: txAction.methodName,
						args: txAction.args,
						gas: txAction.methodName,
						deposit: txAction.attachedDeposit,
					}
				}
		...
		return {
			...
			async requestSignTransactions({ transactions, callbackUrl, meta }) {
				logger.log("SenderWallet:requestSignTransactions", {
					transactions,
					callbackUrl,
					meta,
				});
