...
function setupLedgerWallet(): WalletModule<HardwareWallet> {
	return function LedgerWallet({
		...
		return {
			...
			async requestSignTransactions({ transactions, callbackUrl, meta, accountId }) {
				logger.log("LedgerWallet:requestSignTransactions", {
					transactions,
					callbackUrl,
					meta,
				});

				/// TODO implement above logic, but for multiple txs
				throw new Error("requestSignTransactions not supported for ledger");
