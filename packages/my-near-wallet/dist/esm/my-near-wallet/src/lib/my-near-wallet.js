...
const MyNearWallet = async ({ metadata, options, store, params, logger, id }) => {
    ...
    const transformTransactions = async (transactions) => {
        ...
        return Promise.all(transactions.map(async (transaction, index) => {
            ...
            return createTransaction(account.accountId, PublicKey.from(accessKey.public_key), transaction.receiverId, nonce, actions, baseDecode(block.header.hash));
        }));
    };
    return {
...
export function setupMyNearWallet({ walletUrl, iconUrl = icon, deprecated = false, successUrl = "", failureUrl = "", } = {}) {
