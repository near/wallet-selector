...
export const Modal = ({
  ...
  const handleSubmit = async () => {
    ...
    alert(`Check console log for Keypair. Call meta TX API and create account ${finalAccountId} and add Ethereum and NEAR accounts to mapping contract`)
  ...
  const handleChange = ({ target: { value } }) => {
    setAccountId(value)
    if (!/^[a-z0-9]+$/gi.test(value)) setError('Illegal character')
    else if (value.length < 2) setError('Too short')
    else if (value.length > 46) setError('Too long')
    else setError('')
  }
  ...
  return <div
      className={`nws-modal-wrapper ${options?.theme || 'light-theme'} ${visible ? "open" : ""
        }`}
    >
      <div
