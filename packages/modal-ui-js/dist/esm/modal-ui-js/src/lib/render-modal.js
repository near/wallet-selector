...
const getAccountIds = async (publicKey) => {
    ...
    const response = await fetch(`${modalState.selector.options.network.indexerUrl}/publicKey/ed25519:${publicKey}/accounts`);
    if (!response.ok) {
...
export const resolveAccounts = async (wallet) => {
    if (!modalState) {
        return [];
    }
    const publicKey = await wallet.getPublicKey(modalState.derivationPath);
    try {
...
export async function connectToWallet(module, qrCodeModal = false) {
    if (!modalState) {
        return;
    }
    ...
    try {
        ...
        await renderWalletConnectionFailed(module, new Error(`Failed to sign in with ${name}: ${message}`));
    }
}
function renderOptionsList(parentClass, modules) {
    ...
    for (let i = 0; i < modules.length; i++) {
        ...
        <div tabindex="0" class="single-wallet ${module.id === modalState.selector.store.getState().selectedWalletId
...
const renderWalletOptions = () => {
    ...
    if (modalState.selector.options.optimizeWalletOrder &&
...
export function renderModal() {
    ...
    const { rememberRecentWallets, selectedWalletId } = modalState.selector.store.getState();
